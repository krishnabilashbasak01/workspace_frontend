...
const ReworkSubmitted = () => {
    ...
    useEffect(() => {
        ...
        if (tasksInQueue?.length > 0) {
            ...
            const filteredList = tasksInQueue?.filter((task) => task.status.name === 'Rework Submitted')
    ...
    const ActionButtonComponents = ({ task, index }) => {
        return <div className='flex flex-row gap-1 justify-center'>
            {/* {(user.role.name.toLowerCase() !== 'gd' || user.role.name.toLowerCase() !== 'gd') && <ArrowDownUp className='mr-2 cursor-pointer' />} */}
            {index === 0 && !['ve', 'gd'].includes(user.role.name.toLowerCase()) && <><button disabled={statusChanging}
                onClick={() => { onChangeTaskStatus({ task: task, status: "Rework Need Approval" }) }}
            >{statusChanging ? <Loader2 className={`animate-spin`} /> : <ArrowBigRight size={20} className='mr-2 cursor-pointer' />} </button></>}
...
export default ReworkSubmitted;