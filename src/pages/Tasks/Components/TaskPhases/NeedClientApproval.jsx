...
import { ArrowDownUp, ArrowBigRight, MessageCirclePlus, Loader2, EllipsisVertical, Check, SquareDashed } from 'lucide-react';
...
const NeedClientApproval = () => {
    ...
    useEffect(() => {
        ...
        if (tasksInQueue?.length > 0) {
            ...
            const filteredList = tasksInQueue?.filter((task) => task.status.name === 'Need Client Approval')
    ...
    const ActionButtonComponents = ({ task, index }) => {
        return <div className='flex flex-row gap-1 justify-center'>
            ...
            <button disabled={statusChanging}
                onClick={() => { onChangeTaskStatus({ task: task, status: "Client Approved" }) }}
            >{statusChanging ? <Loader2 className={`animate-spin`} /> : <ArrowBigRight className='mr-2 cursor-pointer' />} </button>
            </>} */}
            {index === 0 && !['gd', 've'].includes(user.role.name.toLowerCase()) && <>
                <Popover>
                        ...
                        <div className="grid gap-4">
                            <div className="space-y-2">
                                ...
                                <p className="text-sm text-muted-foreground">
                                    SME have to take approval from client, if client approved then select "Client Approved" else click on "Need Rework"
                                </p>
                            </div>
                            <div className="flex flex-col">
                                <div className='flex flex-row gap-2' onClick={()=>{onChangeTaskStatus({ task: task, status: "Client Approved" })}}>
                                    <SquareDashed />
                                    <p className={`cursor-pointer inline`}> Client Approved</p>
                                </div>
...
export default NeedClientApproval;