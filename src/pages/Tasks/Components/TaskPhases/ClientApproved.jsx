...
import { ArrowDownUp, ArrowBigRight, MessageCirclePlus, EllipsisVertical, SquareDashed } from 'lucide-react';
...
const ClientApproved = () => {
    ...
    useEffect(() => {
        ...
        if (tasksInQueue?.length > 0) {
            ...
            const filteredList = tasksInQueue?.filter((task) => task.status.name === 'Client Approved')
                    ...
                    <PopoverContent className="w-80">
                        <div className="grid gap-4">
                            ...
                            <div className="flex flex-col">
                                <div className='flex flex-row gap-2' onClick={() => { onChangeTaskStatus({ task: task, status: "Posting Started" }) }}>
                                    <SquareDashed />
                                    <p className={`cursor-pointer inline`}> Posting Started</p>
                                </div>
                                <div className='flex flex-row gap-2' onClick={() => { onChangeTaskStatus({ task: task, status: "Hand Overed To Client" }) }}>
                                    <SquareDashed />
