...
const NeedReWork = () => {
    ...
    useEffect(() => {
        ...
        if (tasksInQueue?.length > 0) {
            ...
            const filteredList = tasksInQueue?.filter((task) => task.status.name === 'Need Rework')
    ...
    const ActionButtonComponents = ({ task, index }) => {
        return <div className='flex flex-row gap-1 justify-center'>
            ...
            {index === 0 && !['sme'].includes(user.role.name.toLowerCase()) && <><button disabled={statusChanging}
                onClick={() => { onChangeTaskStatus({ task: task, status: "Rework In Progress" }) }}
