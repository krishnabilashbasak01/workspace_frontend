...
import { PersonStanding, ChevronRight, Move, Ellipsis, EllipsisVertical, ArrowDownUp, ArrowBigRight, Loader2, MessageCirclePlus } from 'lucide-react';
...
const InQueue = () => {
    ...
    useEffect(() => {
        ...
        if (tasksInQueue?.length > 0) {
            if (user.role.name.toLowerCase() === 've' || user.role.name.toLowerCase() === 'gd') {
                ...
                setTempInQueue(filterTaskInQueue(tasksInQueue, users));
            }
    ...
    const updateReorderOnServer = () => {
        if (socket) {
            socket.emit('on_reorder', { newOrderOfTasks: tasksInQueue, user: user }, (response) => {
    ...
    const ActionButtonComponents = ({ task, index }) => {
        return <div className='flex flex-row gap-1 justify-center'>
            {(user.role.name.toLowerCase() !== 'gd' || user.role.name.toLowerCase() !== 'gd') && <ArrowDownUp size={20} className='mr-2 cursor-pointer' />}
            {index === 0 && user.role.name.toLowerCase() !== 'sme' && <><button disabled={statusChanging}
                onClick={() => { onChangeTaskStatus({ task: task, status: "Work In Progress" }) }}
